...
app.post("/api/weather", async (request, response) => {
  ...
  try {
    const newCheckin = await pool.query(
      "INSERT INTO checkin (nickname, timestamp, lat, lon, country, area) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
      [nickname, timestamp, lat, lon, country, area]
    );
...
app.get("/api/mylogs/:country", async (request, response) => {
  ...
  // get data from postgres DB
  const pgNicknames = await pool.query(
    "SELECT * FROM checkin WHERE country = $1 ORDER BY timestamp DESC LIMIT 20 ",
    [country]
  );
